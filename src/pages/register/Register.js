...
//     setError(false);
//     try {
//       const res = await axios.post("https://blog-backend-m77s.onrender.com/api/auth/register", {
//         username,
//         email,
//         password,
//       });
...
//         {error && <span style={{color:"red", marginTop:"10px"}}>wrong!</span>}
//     </div>
//     )
// }
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "https://blog-backend-m77s.onrender.com/api/auth/register",
        {
          username,
          email,
          password,
  ...
  return (
    <div className="register">
      ...
      {error && (
        <span style={{ color: "red", marginTop: "10px" }}>wrong!</span>
      )}
    </div>
  );
}
